...
description: "Hypergiant stars offer a teleport dialog. The donkey gains energy and grass bonuses, then jumps to any reachable star on a freshly recalculated route."
---
...
<Steps>
  ...
  <Step title="Player chooses YES — resource bonus">
    If the player clicks **YES** (`dialog_closed_status = True`), `_hypergiant_travel()` is called immediately:
  ...
  <Step title="Destination input dialog">
    A second `DialogComponent` opens with `require_input=True`, asking:

    ```
    "Enter destination star:"
    ```
...
The energy cap of 100 prevents overflow. Grass has no upper bound, so a donkey that already carries a large grass reserve will double it. Both bars are refreshed via `_update_status_bars()` immediately after the bonus is applied, so the player sees the change before the destination dialog appears.
